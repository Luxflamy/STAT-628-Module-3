...
def create_late_night_arrival_indicator(df):
    ...
    # Estimate arrival time based on departure time, distance, and average speed
    if 'SCH_DEP_TIME' in df.columns and 'DISTANCE' in df.columns:
        ...
        # For valid departure time and distance, estimate arrival time
        valid_input = ~df['SCH_DEP_TIME'].isna() & ~df['DISTANCE'].isna()
        if valid_input.any():
            # Calculate approximate flight time (assuming average flight speed of 500 miles/hour)
            ...
            # Calculate estimated arrival hour (24-hour format)
            ...
            # Add arrival time period
            df['ARR_TIME_OF_DAY'] = pd.Series(dtype='object')  # Initialize as empty
            
            # Categorize by estimated arrival time
        ...
        # If necessary input data is missing, set default time period
...
def create_arrival_time_block_features(df):
    ...
    # Check if estimated arrival hour is already present
    if 'EST_ARR_DECIMAL_HOUR' not in df.columns:
        # Add default values
        df['ARR_HOUR'] = 12  # Default to noon
        df['ARR_TIME_BLOCK'] = 'Mid-Day (9-12)'  # Default time block
        df['IS_MORNING_RUSH_ARR'] = 0
        df['IS_EVENING_RUSH_ARR'] = 0
        return df

    # Extract hour from estimated arrival time
    df['ARR_HOUR'] = df['EST_ARR_DECIMAL_HOUR'].apply(lambda x: int(x) if not pd.isna(x) else 12)
    
    # Create time blocks (3-hour intervals)
    time_blocks = {
